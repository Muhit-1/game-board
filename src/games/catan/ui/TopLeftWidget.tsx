...
const BUILD_MODES: { mode: BuildMode; label: string; icon: JSX.Element }[] = [
  { mode: 'settlement', label: 'Settlement', icon: <Home size={15} strokeWidth={1.8} /> },
  { mode: 'city', label: 'City', icon: <Building2 size={15} strokeWidth={1.8} /> },
  { mode: 'road', label: 'Road', icon: <Route size={15} strokeWidth={1.8} /> },
  { mode: 'robber', label: 'Robber', icon: <Skull size={15} strokeWidth={1.8} /> },
  { mode: 'erase', label: 'Erase', icon: <Eraser size={15} strokeWidth={1.8} /> },
];

export function TopLeftWidget({ state, dispatch }: { state: GameState; dispatch: Dispatch<GameAction> }) {
  return (
    <Panel title="Current Player & Build" icon={<User size={13} />}>
      <div className="flex flex-col gap-4">
        <div>
          <div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.06em] text-parchment-dim">Current player</div>
            ...
            {state.players.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[11px] font-semibold text-parchment">P{p.slot + 1}</span>
                </button>
              );
            })}
          </div>
        ...
        <div>
          <div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.06em] text-parchment-dim">Build mode</div>
            ...
            {BUILD_MODES.map(({ mode, label, icon }) => {
              ...
              return (
                <button
                  ...
                  className={[
                    'flex items-center gap-2 rounded-lg border px-3 py-2 text-[11.5px] font-semibold transition-colors',
