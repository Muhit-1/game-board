...
export function ScoreContent({ state, dispatch }: { state: GameState; dispatch: Dispatch<GameAction> }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {state.players.map((p) => {
        ...
        return (
          <div key={p.slot} className="group relative rounded-lg border border-[var(--line)] bg-charred-oak/40 p-2.5 [@media(hover:none)]:mb-6">
            ...
            {/* Hover-reveal on a mouse; always visible on touch, since there's no hover to trigger it */}
            <div className="pointer-events-none absolute left-0 right-0 top-full z-20 mt-1 rounded-md border border-brass/40 bg-charred-oak px-2 py-1.5 text-center font-mono text-[12px] text-parchment opacity-0 shadow-deep transition-opacity duration-100 group-hover:opacity-100 [@media(hover:none)]:opacity-100">
...
export function ScorePanel({ state, dispatch }: { state: GameState; dispatch: Dispatch<GameAction> }) {
  return (
    <Panel title="Score" icon={<Trophy size={14} />} width={280} collapsible={false} scrollable={false}>
      <ScoreContent state={state} dispatch={dispatch} />
    </Panel>
  );
}
